...
export default function Home() {
  ...
  const [step, setStep] = useState<'inicio' | 'cadastro' | 'autoconhecimento' | 'resultado' | 'boasVindas' | 'home' | 'trilhas' | 'trilhaDetalhes' | 'conviteProfissionais'>('inicio')
        ...
        <section className="w-full max-w-3xl bg-zinc-900 p-8 rounded-xl shadow-xl space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-zinc-300">
            <div onClick={() => setStep('trilhas')} className="bg-zinc-800 p-4 rounded-xl hover:bg-zinc-700 transition cursor-pointer">
              <h3 className="text-green-400 font-semibold">Trilhas de Autodesenvolvimento</h3>
              <p>Explore conteúdos direcionados ao seu perfil</p>
            </div>
            <div onClick={() => setStep('conviteProfissionais')} className="bg-zinc-800 p-4 rounded-xl hover:bg-zinc-700 transition cursor-pointer">
              <h3 className="text-green-400 font-semibold">Seja Psicólogo Parceiro</h3>
              <p>Convidamos profissionais com propósito</p>
            </div>
          </div>
        </section>
      )}

      {step === 'conviteProfissionais' && (
        <section className="w-full max-w-xl bg-zinc-900 p-6 rounded-xl shadow-xl text-center space-y-4">
          <h2 className="text-2xl font-bold text-green-400">Convite aos Profissionais</h2>
          <p className="text-zinc-300">Se você é um profissional da saúde mental e deseja fazer parte da nossa missão de transformar a vida de jovens por todo o Brasil, envie seu currículo para:</p>
          <p className="text-green-400 font-semibold">equipejovify@gmail.com</p>
          <p className="text-zinc-300">Buscamos pessoas que compartilham do nosso propósito e acreditam na importância do cuidado emocional dos jovens.</p>
